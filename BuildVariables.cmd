set GAMEPATH=C:\Program Files\The Witcher 3 Wild Hunt
set WITCHERBASEDIR=D:\Games\Witcher3\W3UncookedBnW
set MODKITPATH=C:\Program Files\The Witcher 3 Wild Hunt\Witcher 3 Mod Kit\bin\x64
set W3STRINGSENCODER=C:\Program Files\The Witcher 3 Wild Hunt\Witcher 3 W3Strings Encoder

set W3STRINGSIDSPACE=0956
set NAME=modReflexes

set MODPATH=%~dp0
set BUNDLES=%MODPATH%Bundles
set MODDED=%BUNDLES%\1_Modded
set COOKED=%BUNDLES%\2_Cooked
set PACKED=%MODPATH%Compiled\%NAME%\content
set PACKEDSCRIPTS=%MODPATH%Compiled\%NAME%
set COMPILED=%MODPATH%Compiled
set STRINGS=%MODPATH%W3Strings
set SCRIPTS=%MODPATH%Script